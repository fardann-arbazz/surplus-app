<button id="locationTrigger" onclick="LocationPicker.open()"
    class="flex items-center gap-2 px-3 py-2 rounded-full bg-base-200/70 hover:bg-base-300/80 
           border border-base-300/50 text-xs transition-all duration-200 hover:shadow-sm"
    style="display: none;">
    {{-- Ikon pin --}}
    <svg class="w-4 h-4 text-warning shrink-0" fill="currentColor" viewBox="0 0 24 24">
        <path
            d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5s2.5 1.12 2.5 2.5S13.38 11.5 12 11.5z" />
    </svg>
    {{-- Teks lokasi --}}
    <span id="locText" class="max-w-30 truncate font-medium text-base-content/80">Lokasi tersimpan</span>
    {{-- Status dot --}}
    <span id="locDot" class="w-2 h-2 rounded-full bg-success shrink-0"></span>
    {{-- Chevron --}}
...
{{-- Full-Screen Modal --}}
<div id="locationModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-red-500 mt-125"
...
@push('scripts')
    ...
    <script>
        const LocationPicker = (() => {
            ...
            function init() {
                console.log('LocationPicker init');
                ...
                fetch(ROUTES.locationStatus, {
                    ...
                    .then(data => {
                        ...
                        if (data.has_location) {
                            ...
                            const triggerBtn = document.getElementById('locationTrigger');
            ...
            function open(forced = false) {
                console.log('LocationPicker.open() called, forced:', forced);
            ...
            function updatePendingCoords(lat, lng) {
                ...
                if (preview) {
                    preview.innerHTML = '<span class="loading loading-dots loading-xs text-warning"></span>';
                }
                geocodeTimer = setTimeout(() => {
                    reverseGeocode(lat, lng, addr => {
                        if (preview) {
                            preview.textContent = addr || 'Alamat tidak ditemukan';
                        }
                    });
                }, 500);
            }
            ...
            function setupSearchListeners() {
                ...
                document.addEventListener('click', function(e) {
                    if (!e.target.closest('#searchSuggestions') && !e.target.closest(
                            '#locationSearchInput')) {
                        suggestions.classList.add('hidden');
                        selectedSuggestionIndex = -1;
                    }
                    ...
                    .then(data => {
                        if (!data.length) {
                            suggestions.innerHTML =
                                `<div class="px-4 py-8 text-center"><p class="text-sm text-base-content/50">Alamat tidak ditemukan</p></div>`;
                        ...
                        suggestions.innerHTML = data.map(item => {
                            ...
                            return `<div class="suggestion-item" data-lat="${item.lat}" data-lon="${item.lon}" onclick="LocationPicker.selectSuggestion(${item.lat}, ${item.lon}, '${escapeHtml(item.display_name)}')">
                                <div class="suggestion-icon"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><!--!Font Awesome Free v7.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2026 Fonticons, Inc.--><path d="M128 252.6C128 148.4 214 64 320 64C426 64 512 148.4 512 252.6C512 371.9 391.8 514.9 341.6 569.4C329.8 582.2 310.1 582.2 298.3 569.4C248.1 514.9 127.9 371.9 127.9 252.6zM320 320C355.3 320 384 291.3 384 256C384 220.7 355.3 192 320 192C284.7 192 256 220.7 256 256C256 291.3 284.7 320 320 320z"/></svg></div>
            ...
            function confirm() {
                ...
                fetch(ROUTES.locationUpdate, {
                    ...
                    .then(({
                        ...
                        btn.disabled = false;
                        btn.innerHTML = '✅ Konfirmasi Lokasi';
                        if (status !== 200) {
            ...
            function setLocBadge(state, text) {
                ...
                if (dot) dot.className = state === 'on' ? 'w-2 h-2 rounded-full bg-success' :
